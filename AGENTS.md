...
- **内嵌素材**：`levels.py` 的 `embed` 字段 → 前端 iframe 渲染；**素材路径必须在 `config.EMBEDDABLE_STATIC_HTML` 白名单内**（否则被安全头拦截报 ERR_BLOCKED_BY_RESPONSE）；素材页脚本一律外置 .js（CSP `script-src 'self'` 禁内联）；素材页需要的谜题数据由服务端生成后注入（L2 摩斯点划串由答案生成、经 `signal` 字段下发，app.js 在 iframe onload 后 `setSignal()` 注入）——**客户端静态源码不得出现明文答案/解题用数据表**（L1 的 zua 注释、L6 的 SVG 注释是设计内的明面线索，属例外）
